...
Page({
  ...
  analyze(){
    ...
    this.data.red.forEach((item, index)=>{
      if(item.checked){
        red.push( index+1 )
      }
    })

    this.data.blue.forEach((item, index) => {
      if (item.checked) {
        blue.push(index + 1)
      }
    })
    ...
    wx.navigateTo({
      url: '/pages/ssq/comparer-result/comparer-result?red=' + this.oneToTwo(red) + '&blue=' + this.oneToTwo(blue),
    })  
